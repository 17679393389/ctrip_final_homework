...
export default function Diary() {
  ...
  const getDiaryList = async (data: DiaryList) => {
    try{
      ...
      let diaryInfoList = [];
      let invalidDiaryCount = 0;  //记录被删除的游记数量
      ...
      setTableParams({
        ...
        pagination: {
          ...
          total: res.data.totalCount-invalidDiaryCount,
        },
      })
    } catch (err) {
      setLoading(false);
    ...
    const columns: ColumnsType<DiaryType> = [
        { title: '', dataIndex: 'diaryInfo',width:80,render: (_,diary) => (
          <Image.PreviewGroup items={diary.photoList}>
            ...
            // <Card bordered={false} className="w-full" bodyStyle={{ padding: 8 }}>
            //     <Row>
            //         <Col span={4}>
            //             <Image.PreviewGroup items={diary.photoList}>
            //                 <Image className="rounded object-cover"
            ...
            //             </Image.PreviewGroup>
            //         </Col>
            //         <Col span={20}  onClick={() => onDetail(diary)}>
            //         <Row><Title level={5}>{diary.diaryInfo.title}</Title></Row>
            //         <Row>
            //             <Paragraph className='text-gray-700' ellipsis={{ rows: 2 }}>
            //             {(diary.diaryInfo.content).repeat(50)}
            //             </Paragraph>
            //         </Row>
            //         </Col>
            //     </Row>
